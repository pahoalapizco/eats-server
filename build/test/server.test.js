...
var HOST = "http://localhost:5000/graphql"; // Variables para _id's que se requieren en test posteriores,

var token, restauranteID, categoriaID, platilloID, repartidorID, pedidoID;
describe("[USUARIOS]", function () {
  test("Crear usuario", function (done) {
    var json = {
  ...
  test("Usuario creado inicia sesión", function (done) {
    var json = {
  ...
  test("Debe obtener al usuario en sesión", function (done) {
    var json = {
    ...
    request.post({
      ...
      expect(res.statusCode).toBe(200);
      body.should.have.property("data");
      expect(body.data.getUser).not.toBeNull(); // body.data.should.have.property('getUser').with.lengthOf(1)

      done(err);
    });
  });
});
describe("[REPARTIDORES]", function () {
  test("Debe crear un repartidor", function (done) {
    var json = {
      query: "mutation($data:RepartidorInput){ addRepartidor(data:$data){ _id, vehiculo } }",
      variables: {
        data: {
          name: "user test",
          lastname: "jest test",
  ...
  test('Debe obtener un arreglo de repartidores', function (done) {
    var json = {
...
describe("[RESTAURANTES]", function () {
  test("Debe crear un restaurante", function (done) {
    var json = {
      query: "mutation($data:RestaurantInput){ addRestaurant(data:$data){ _id, name } }",
      variables: {
        data: {
  ...
  test("Debe obtener un arreglo de restaurantes", function (done) {
    var json = {
...
describe("[CATEGORIAS]", function () {
  test("Crear categoria", function (done) {
    var json = {
      query: "mutation($data:CategoriaInput){ addCategoria(data:$data){ _id, name } }",
      variables: {
        data: {
          name: "categoria test"
        }
  ...
  test("Debe obtener un arrego de categorias", function (done) {
    var json = {
...
describe('[PLATILLOS]', function () {
  test('Debe agregar un platillo', function (done) {
    var json = {
      query: "mutation($data:PlatilloInput){ addPlatillo(data:$data){ _id, name } }",
      variables: {
        data: {
          name: "Platillo test",
  ...
  test('Debe regresar un arreglo de platillos', function (done) {
    var json = {
  ...
  test('Debe regresar un obteto Platilo', function (done) {
    var json = {
...
describe('[PEDIDOS]', function () {
  test('Debe registrar un pedido', function (done) {
    var json = {
      query: "mutation($data:PedidoInput){ addPedido(data:$data){ _id, detail { platillo {_id} } } }",
      variables: {
        data: {
          restaurant: restauranteID,
          total: 20,
  ...
  test('Debe obtener un arreglo de pedidos', function (done) {
    var json = {
    ...
    request.post({
      ...
      body.data.should.have.property('getPedidos')["with"].lengthOf(1); // body.data.getPedidos[0].should.have.property('detail').with.lengthOf(1)
